CREATE VIEW `vista_productos` AS
 ...
 FROM `productos`
  ...
  `productos`.`unidadmedidaventas`,
  `ventas`.`abreviatura` AS `nombunidmedventas` ,
  `productos`.`preciounidad` AS `preciocosto`,
  `productos`.`ganancia`,
  `productos`.`tieneimpuesto`,
  `productos`.`impuesto`,
  IF(`productos`.`tieneimpuesto` = 1, (`productos`.`preciounidad`*(1+(`productos`.`ganancia`/100)))*(1+(`productos`.`impuesto`/100)),(`productos`.`preciounidad`*(1+(`productos`.`ganancia`/100)))) AS `preciounidad`,
  `productos`.`unidadmedidaalmacen`,
  `almacen`.`abreviatura` AS `nombunidmedalmacen`,
  `productos`.`relacionunidad`,
  `productos`.`imagen` AS `imagen`,
