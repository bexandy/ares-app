...
class MarcaController extends AbstractActionController
{
                ...
                if (!$adapter->isValid())
                {
                    ...
                    if ($File['error'] !== 4) {
                        ...
                        if ($adapter->receive($file_name)) {
                            ...
                            $marca->setImagen($rutaImagen);
                        }
                    }
                        $this->getMarcaMapper()->saveMarca($marca);

                        echo ("<script type='text/javascript' >alert('Marca Editada');</script>");
                        return $this->redirect()->toRoute('marca');
                }
            }
        }
    ...
    public function editAction()
    {
        ...
        if (!$id) {
            return $this->redirect()->toRoute('marca', array('action'=>'add'));
        }
        $marca = $this->getMarcaMapper()->getMarca($id);
